...
export const SumitomoMoldingMachine = (props) => {
    ...
    const blinkRef = useRef(false); // Ref untuk menyimpan state blinking
    const redLightMaterialRef = useRef(null); // Ref untuk menyimpan material lampu merah
    ...
    useFrame((state) => {
        // console.log("useFrame is running", state);
        if (blinkRef.current && redLightMaterialRef.current) {
            // Hitung intensitas emissive berdasarkan waktu
            const intensity = Math.sin(state.clock.elapsedTime * 10) * 0.5 + 0.5; // Sinus untuk efek kedip
            redLightMaterialRef.current.emissiveIntensity = intensity * 5; // Skala ke intensitas yang diinginkan
        }
    });
